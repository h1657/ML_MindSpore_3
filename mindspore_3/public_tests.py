...
def test_c1(model):
    assert len(model) == 3, \
        f"Wrong number of layers. Expected 3 but got {len(model)}"
    assert model[0].weight.shape[1] == 400, \
        f"Wrong input shape. Expected [None,  400] but got {model[0].weight.shape[1]}"
    
    expected_shapes = [[None, 25], [None, 15], [None, 1]]
    expected_activations = [nn.Sigmoid, nn.Sigmoid, nn.Sigmoid]

    # 遍历模型的各个层
    for i, layer in enumerate(model):
        assert isinstance(layer, nn.Dense), \
            f"Wrong type in layer {i}. Expected nn.Dense but got {type(layer)}"
        
        assert layer.weight.shape[0] == expected_shapes[i][-1], \
            f"Wrong number of units in layer {i}. Expected {expected_shapes[i][-1]} but got {layer.weight.shape[-1]}"
        
        assert isinstance(layer.activation, expected_activations[i]), \
            f"Wrong activation in layer {i}. Expected {expected_activations[i]} but got {type(layer.activation)}"
