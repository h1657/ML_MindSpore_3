...
def plt_act_trio():
    ...
    ax[1].plot(X, sigmoid_fn(Tensor(X, dtype=ms.float32)).asnumpy())
    ax[1].axvline(0, lw=0.3, c="black")
    ax[1].axhline(0, lw=0.3, c="black")
    ax[1].set_title("Sigmoid")
    relu_fn = nn.ReLU()
    ax[2].plot(X, relu_fn(Tensor(X, dtype=ms.float32)).asnumpy()) 
...
def display_errors(model,X,y):
    ...
    if len(idxs) == 0:
        ...
        for i in range(cnt):
            j = int(idxs[i])
            # X = X.asnumpy()
            X_reshaped = X[j].reshape((20,20)).T.asnumpy()

            # Display the image
            ax[i].imshow(X_reshaped, cmap='gray')

            # Predict using the Neural Network
            prediction = model.predict(Tensor(X[j].reshape(1,400)))
            softmax = ops.Softmax(axis=-1) 
            prediction_p = softmax(prediction).asnumpy()
...
def plot_loss_tf(history):
    ...
    ax.set_xlim([0, len(history)])  # 横坐标范围
    ax.set_ylim([min(history) * 0.9, max(history) * 1.1])
    ax.set_xlabel('Epoch')
